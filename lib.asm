...
print_uint:
    ...
    mov rax, rdi      ; Берем беззнаковое 64-битное число
    mov rcx, 10       ; Делитель
    sub rsp, 32       ; Резервируем место для строки (большая длина, чем максимальное число цифр)
    mov rbx, rsp      ; Указываем на буфер для строки
    mov rsi, 0        ; Индекс для заполнения буфера
.loop:
    xor rdx, rdx       ; Обнуляем rdx перед делением
    div rcx             ; rax = rax / rcx, rdx = rax % rcx
    add dl, '0'        ; Преобразуем остаток в ASCII
    mov [rbx + rsi], dl; Сохраняем цифру в буфер
    inc rsi            ; Увеличиваем индекс
    cmp rax, 0        ; Проверяем, не закончилась ли цифра
    jnz .loop          ; Если не закончилась, продолжаем цикл

    ; Добавляем символ конца строки
    mov byte [rbx + rsi], 0

    ; Печатаем результат
    mov rdi, rbx      ; Указываем на начало строки
    mov rsi, rsi      ; Первая аргумент передаем в print_string через rsi
    call print_string  ; Вызываем функцию печати

    add rsp, 32       ; Освобождаем место на стеке
