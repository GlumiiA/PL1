...
read_word:
    ; Читаем символ и сохраняем его
.loop:
    call read_char      ; Читаем символ
    cmp rax, 0          ; Проверяем, нет ли ошибки
    je .end             ; Если ошибка, выходим
    cmp al, 0x20        ; Проверяем, пробел ли это
    je .end             ; Если пробел, завершаем
    mov [rsi], al       ; Сохраняем символ в буфере
    inc rsi             ; Переходим к следующему месту в буфере
    inc rdi             ; Увеличиваем длину слова
    jmp .loop           ; Повторяем цикл

.end:
    mov byte [rsi], 0   ; Добавляем нуль-терминатор
    mov rax, rdi        ; Возвращаем длину слова
    ret
