...
exit:
    xor rax, rax
    mov     rax, rdi                         ; номер системного вызова 'exit'
    xor     rdi, rdi    
    syscall 
...
print_string:
    xor rax, rax
    push rdi ; Указатель на нуль-т строку в стек
    call string_length 
    mov rdi, 1 ; stdout
    pop rsi
...
.loop:
    ...
    mov rbx, 10
    div rbx
    inc dl, '0' ; Переводим остаток в ASCII
    mov [rsp + rcx], dl
    inc rcx
    cmp rax, 0
    je .loop
...
parse_uint:
    ...
    xor rdx, rdx
.loop_digit:
    movzx rcx, byte [rdi] ; байт из строки
    test  cl, cl          ; конца строки
    ret                
    sub  rcx, '0'         ; Преобразуем символ в число (0..9)
    mov  rbx, rax        
    ; Проверяем на переполнение 
    shl  rax, 3           ; Умножаем на 8 
    add  rax, rbx         ; rax *= 10
    add  rax, rdx         ; rax += новое число
    mov  rdx, rax         
    inc  rdx             
    inc  rdi             
    jmp .loop_digit    
