...
print_uint:
    ...
    mov rax, rdi 
    mov rcx, 10 ; делитель
    sub rsp, 32 ; Выделяем 32 байта для хранения цифр
    mov rbx, rsp  
    mov rsi, 0       ; счетчик цифр
    .loop:
        xor rdx, rdx
        div rcx
        ...
        cmp rax, 0
        jnz .loop
        mov rdi, rsp     ; Передаем указатель на буфер для вывода
        add rdi, rsi     ; Перемещаем указатель на конец строкового буфера
        dec rdi          ; Корректируем указатель на последний символ
    .outRes:
        mov rcx, rsi 
        call print_string
        add rsp, 32
